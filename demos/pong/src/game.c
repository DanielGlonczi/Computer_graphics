    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_KEYDOWN:
            ...
            case SDL_SCANCODE_S:
                set_left_pad_speed(&(game->pong), +300);
                break;
            case SDL_SCANCODE_Z: /* increase ball radius */
                if (game->pong.ball.radius < 120.0f) {
                    game->pong.ball.radius += 5.0f;
                }
                break;
            case SDL_SCANCODE_X: /* decrease ball radius */
                if (game->pong.ball.radius > 5.0f) {
                    game->pong.ball.radius -= 5.0f;
                }
                break;
            default:
...
void render_game(Game* game)
{
    ...
    {
        char title[128];
        snprintf(title, sizeof(title), "Pong - Left: %d Right: %d", game->pong.left_score, game->pong.right_score);
